...
def run_camera_detection(camera_index: int = 0, model_name: str = 'buffalo_l', provider: str = 'CUDAExecutionProvider', det_size: int = 640, device: int = 0, max_fps: float = 0.0):
    ...
    if 'CUDA' in provider.upper():
        ctx_id = int(device)
    app.prepare(ctx_id=ctx_id, det_size=det_size_value)
...
if __name__ == '__main__':
    ...
    run_camera_detection(camera_index=args.camera, model_name=args.model, provider=args.provider, det_size=args.det_size, device=args.device, max_fps=args.max_fps)
